...
import {getSchemasUsingGet, previewDataUsingPost} from "@/services/hwqbi/dataSourceController";
...
const MyLayout = () => {
  ...
  const fetchSchemas = async () => {
    const res = await getSchemasUsingGet({id: params.id})
    if (res.data) {
      const eles = []
  ...
  const fetchPreviewData = async (tableName : string) => {
    const res = await previewDataUsingPost({
    ...
    <Layout style={{ minHeight: '100vh' ,margin: '-32px -40px'}}>
      <Sider width={250} style={{ background: '#fff' }}>
        ...
        <div style={{padding: '16px'}}>
          <h4 style={{paddingBottom: '10px', paddingTop: '20px'}}>数据表</h4>
          <Input placeholder={'通过表名搜索'} addonBefore={<SearchOutlined/>}></Input>
        </div>
